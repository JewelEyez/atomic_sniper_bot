...
const desiredPrice = "100000000"; //The price you want to buy it for, in WAX
const privateKeys = ["5JvrmPq1zwjmuNWF8urW6EiLoxfMi9mG1TJjkFYD7T3xKhmmw9L"];
...
async function getSales() {
  const url =
    "https://wax.api.atomicassets.io/atomicmarket/v2/sales?asset_id=" +
    assetID +
...
getSales().then((data) => {
  Object.keys(data.data).forEach((key) => {
    ...
    let usdToPay = usdPrice.slice(0, -8) + "." + usdPrice.slice(-8) + " WAX"; //the amount of wax to deposit for USD sales
    console.log("\nUSD TO PAY: " + usdToPay);
    ...
    else if ((symbol == "WAX") & (price > desiredPrice) & (state == 1)) {
      console.log("\nBad deal!");
    ...
    else if ((symbol == "USD") & (usdPrice <= desiredPrice) & (state == 1)) {
      let priceToAssert = price.slice(0, -2) + "." + price.slice(-2) + " USD";
